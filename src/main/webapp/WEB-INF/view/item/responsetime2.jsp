...
Long totalMinTime = (Long) request.getAttribute("totalMinTime");
Long totalMaxTime = (Long) request.getAttribute("totalMaxTime");
Long totalAvgTime = (Long) request.getAttribute("totalAvgTime");

int radius = 4, step = 1;
if (Integer.parseInt(last) < 7) { // 0, 1
	step = 1;
	radius = 4;
} else if (Integer.parseInt(last) < 15) { // 7
	step = 3;
	radius = 3;
} else if (Integer.parseInt(last) < 30) { // 15
	step = 12;
	radius = 2;
} else { // 30
	step = 12;
...
<div class="bg01">
  ...
  <tr>
	...
	HttpOption httpOption = (HttpOption) JSONObject.toBean(optionJSON, HttpOption.class);  			
  	%>
    <td class="noline">超时时间：</td>
    <td class="noline"><%=httpOption.getHttpTimeout() %>毫秒</td>
  </tr>
</table>
      ...
      <div class="w_960 pt">
           <div class="w_left">
              ...
              <ul class="list">
                  <li><span class="point">&nbsp;</span><a href="/item/usability2.do?itemId=<%=item.getId()%>">可用性统计</a></li> 
                  <li class="current"><span class="point">&nbsp;</span><a href="#">响应时间统计</a></li>
                  <li class="noline"><span class="point">&nbsp;</span><a href="/item/dailystat2.do?itemId=<%=item.getId()%>">每日统计</a></li>
              </ul>
             </div>
          ...
          <div class="w_735">
              ...
              <p class="fr pr">
              		<span><a <%=( last.equals("0") ? "class=\"on\"" : "")%> href="/item/responsetime2.do?itemId=<%=item.getId()%>">今日</a></span>
              		<span><a <%=( last.equals("1") ? "class=\"on\"" : "")%> href="/item/responsetime2.do?itemId=<%=item.getId()%>&last=1">昨日</a></span>
              		<span><a <%=( last.equals("7") ? "class=\"on\"" : "")%> href="/item/responsetime2.do?itemId=<%=item.getId()%>&last=7">最近7天</a></span>
              		<span><a <%=( last.equals("15") ? "class=\"on\"" : "")%> href="/item/responsetime2.do?itemId=<%=item.getId()%>&last=15">最近15天</a></span>
              		<span><a <%=( last.equals("30") ? "class=\"on\"" : "")%> href="/item/responsetime2.do?itemId=<%=item.getId()%>&last=30">最近30天</a></span>
      ...
      <table class="tu" cellpadding="0" cellspacing="0" border="0" style="width: 730px; margin-top: 10px; margin-bottom: 20px;">
              ...
              for (int i = 0;i<keyList.size();i++) {
            	  ...
            	  String avgTimeStr = "-";
            	  if (jobj != null) {
            		  avgTimeStr = jobj.getLong("avg") + "ms";
            	  }
              ...
              <tr >
                  <td>&gt;1500ms</td>
                  <td><%=classCountArr[5] %></td>
                  <td><%=CommonUtil.get2pDouble(classCountArr[5] * 100. / classCountTotal)%>%</td>
              </tr>
            </table>
            </td></tr>  
              </table>
