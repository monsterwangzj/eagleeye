...
function updateChart(domId){
    $.get(
	    'http://localhost/chartdata.jsp',
	    function(data) { 
	    	var xmltext = xmlToString(data);
	    	// alert(xmltext); 
	    	updateChartXML("myFirst", xmltext);
	    }
    );
}

$(document).ready(function() {
	// setInterval(updateChart, 5 * 60 * 1000); // 5 分钟
	// setInterval(updateChart, 15 * 1000);
});
...
<% 
String dataurl = "/item/detailusability2.do?itemId=" + item.getId();
//dataurl = "/test/test2.json";
...
<div class="bg01">
  ...
  <tr>
    ...
    if (item.getType() == Item.TYPE_HTTP) {
    	HttpOption httpOption = (HttpOption) JSONObject.toBean(options, HttpOption.class);
    	timeout = httpOption.getHttpTimeout();
    }
	%>
    <td class="noline"><%=timeout %>毫秒</td>
  </tr>
</table>
      ...
      <div class="w_960 pt">
           <div class="w_left">
              ...
              <ul class="list">
                  ...
                  <li><span class="point">&nbsp;</span><a href="/item/responsetime2.do?itemId=<%=item.getId()%>">响应时间统计</a></li>
          ...
          <div class="w_735">
              <div class="title01">
	              ...
	              <p class="fr pr">
	              	<span><a <%=( last.equals("0") ? "class=\"on\"" : "")%> href="/item/usability2.do?itemId=<%=item.getId()%>">今日</a></span>
	              	<span><a <%=( last.equals("1") ? "class=\"on\"" : "")%> href="/item/usability2.do?itemId=<%=item.getId()%>&last=1">昨日</a></span>
	              	<span><a <%=( last.equals("7") ? "class=\"on\"" : "")%> href="/item/usability2.do?itemId=<%=item.getId()%>&last=7">最近7天</a></span>
	              	<span><a <%=( last.equals("15") ? "class=\"on\"" : "")%> href="/item/usability2.do?itemId=<%=item.getId()%>&last=15">最近15天</a></span>
	              	<span><a <%=( last.equals("30") ? "class=\"on\"" : "")%> href="/item/usability2.do?itemId=<%=item.getId()%>&last=30">最近30天</a></span>
