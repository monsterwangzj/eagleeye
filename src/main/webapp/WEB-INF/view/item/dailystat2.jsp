...
%><%@ page import="com.chengyi.eagleeye.model.assist.HttpOption,java.text.NumberFormat,java.util.*,com.chengyi.eagleeye.util.CommonUtil,net.sf.json.JSONObject,com.chengyi.eagleeye.model.*,org.apache.commons.collections.CollectionUtils" 
%><%
    ...
    <td class="noline"><%=httpOption.getHttpTimeout() %>毫秒</td>
      ...
      <div class="w_960 pt">
          ...
          <div class="w_735">
              ...
              <p class="fr pr">
              		<span><a <%=( daysMode.equals("7") ? "class=\"on\"" : "")%> href="/item/dailystat2.do?itemId=<%=item.getId()%>&mode=7">最近7天</a></span>
              		<span><a <%=( daysMode.equals("15") ? "class=\"on\"" : "")%> href="/item/dailystat2.do?itemId=<%=item.getId()%>&mode=15">最近15天</a></span>
              		<span><a <%=( daysMode.equals("30") ? "class=\"on\"" : "")%> href="/item/dailystat2.do?itemId=<%=item.getId()%>&mode=30">最近30天</a></span>
              ...
              if (keyList != null && keyList.size() > 0) {
            	     for (int k = keyList.size() - 1; k > 0; k--) {
		 			 ...
		 			 <tr>
		                  <td><%=key%></td>
		                  <td><%=jobj.getLong("usability") %>%</td>
		                  <td><%=errorTime %></td>
		                  <td><%=jobj.getLong("min") %>ms</td>
		                  <td><%=jobj.getLong("avg") %>ms</td>
		                  <td><%=jobj.getLong("max") %>ms</td>
		              </tr>
 					 <%}
