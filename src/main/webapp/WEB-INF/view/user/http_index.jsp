...
HashMap<Long, Long> itemGroupMap = (HashMap<Long, Long>) request.getAttribute("itemGroupMap");
...
if (tid == null) {
	if (gid == null) {
		pageLinkBase = "/monitor/index.do?p=";
	} else {
		pageLinkBase = "/monitor/index.do?g=" + gid + "&p=";
	}
} else {
	pageLinkBase = "/monitor/index.do?tid=" + tid + "&p=";
}
...
<div class="bg01">
    <div class="con">
      ...
      <div class="w_960 pt">
           <div class="w_left">
             <div class="w_210 bc">
              <div class="title"><h4>按类型查看</h4></div>
              <ul id="deleteGroup" class="list01">
                  <li data="Http" <%if (tid != null && tid.equals("http")) {out.print(" class='current'");} %>>
            	      <span class="point">&nbsp;</span>
            		  <a href="/monitor/index.do?tid=http">Http (<%=(itemTypeMap.get(new Long(Item.TYPE_HTTP)) == null ? 0 : itemTypeMap.get(new Long(Item.TYPE_HTTP)))%>)</a>
            	  </li>
            	  <li data="Ping" <%if (tid != null && tid.equals("ping")) {out.print(" class='current'");} %>>
            		  <span class="point">&nbsp;</span>
            		  <a href="/monitor/index.do?tid=ping">Ping (<%=(itemTypeMap.get(new Long(Item.TYPE_PING)) == null ? 0 : itemTypeMap.get(new Long(Item.TYPE_PING)))%>)</a>
            	  </li>
             </ul>
             </div>
             &nbsp;<br/>
             <div class="w_210 bc">
              <div class="title"><h4>网站监控分类</h4></div>
              <ul id="createGroup" class="list no">
                  <li><span><img src="/styles/images/add.gif" /></span><a href="/itemgroup/new.do">创建自定义分类</a></li>
              </ul>
              <ul id="deleteGroup" class="list01">
              	...
              	<% } else { %><%--
              		...
              		<%for (ItemGroup itemGroup: itemGroups) { %>
              			<li data="<%=itemGroup.getId()%>">
              				<span class="point">&nbsp;</span>
              				<a href="/monitor/index.do?g=<%=itemGroup.getId()%>"><%=itemGroup.getName()%> (<%=itemGroupMap.get(itemGroup.getId())%>)</a>
              				<a class="set" href="/itemgroup/edit.do?gid=<%=itemGroup.getId()%>" data="<%=itemGroup.getName() %>" title="修改">&nbsp;</a>
          ...
          <div class="w_735">
              ...
              <table id="box01" class="gray" cellpadding="0" cellspacing="0" border="0">
              <tr class="tit">
                  <td width="20%">项目名称</td>
                  <td width="8%">项目<br/>类型</td>
                  <td width="32%">URL</td>
                  <td width="10%">监控频率</td>
                  <td width="8%">可用率</td>
                  <td width="12%">平均响应<br/>时间</td>
                  <td colspan="3" width="10%">操作</td>
              </tr>
			  ...
			  <% } else {
				  	...
				  	String itemType = "http";
				  	String linkUrl = "/item/usability2.do?itemId=" + item.getId();
				  	if (item.getType() == Item.TYPE_PING) {
				  		linkUrl = "/pingitem/usability2.do?itemId=" + item.getId();
				  		itemType = "ping";
				  	}
	                  ...
	                  <td><%=itemType%></td>
	                  <td class="url"><div><%=item.getUri()%></div></td>
	                  ...
	                  <%} %>
	                  <%
	                  String editLink = "/item/edit.do?id=" + item.getId();
	                  if (item.getType() == Item.TYPE_PING) {
	                	  editLink = "/pingitem/edit.do?id=" + item.getId();
	                  }
					  %>
...
$("#gchange").click(function() {
	...
	$.get(base + "/itemgroup/update.do", {"gname": name, "gid": layer2.gid}, function(results) {
		results = eval("(" + results + ")");
		if(results.status == 1) {
			layer2.hide();
			layer3.un().show(function(el) {
...
$("#gdelete").click(function() {
	$.get(base + "/itemgroup/del.do", {"gid": layer2.gid}, function(results) {
		results = eval("(" + results + ")");
		if(results.status == 1) {
			layer2.hide();
			layer3.un().show(function(el) {
