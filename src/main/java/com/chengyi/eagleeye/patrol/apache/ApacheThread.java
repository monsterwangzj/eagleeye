...
public class ApacheThread implements Runnable {
	private static final Logger logger = Logger.getLogger(ApacheThread.class);
	...
	public ApacheThread(Item item, ApacheParam apacheParam) {
		this.item = item;
		...
		while (true) {
			...
			if (nginxResult2.getStatus() != NginxResult.STATUS_OK) {
				...
				if (retryThread == null || !retryThread.isAlive()) {
					retryThread = new Thread(new RetryApacheThread(item, apacheParam));
					retryThread.start();
				}
