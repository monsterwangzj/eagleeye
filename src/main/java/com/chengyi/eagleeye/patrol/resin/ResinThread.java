package com.chengyi.eagleeye.patrol.resin;
...
public class ResinThread implements Runnable {
	private static final Logger logger = Logger.getLogger(ResinThread.class);
	...
	public ResinThread(Item item, ApacheParam apacheParam) {
		this.item = item;
		...
		while (true) {
			...
			if (nginxResult2.getStatus() != NginxResult.STATUS_OK) {
				...
				if (retryThread == null || !retryThread.isAlive()) {
					retryThread = new Thread(new RetryResinThread(item, apacheParam));
					retryThread.start();
				}
